 ...
 */
    ...
    async function initializeSectorOpsView() {
        ...
        try {
            ...
            if (!document.getElementById('aircraft-info-window')) {
                 // NEW: Inject only the main container. The content will be fully managed by JavaScript.
                 const windowHtml = `
    ...
    async function handleAircraftClick(flightProps, sessionId) {
        ...
        // ================== START: FIX 2 ==================
        // NEW: Target the main window container directly.
        const aircraftInfoWindow = document.getElementById('aircraft-info-window');
        aircraftInfoWindow.innerHTML = `<div class="spinner-small" style="margin: 2rem auto;"></div><p style="text-align: center;">Loading flight data...</p>`;
        // =================== END: FIX 2 ===================
        ...
        try {
            ...
            // MODIFIED: Target the correct element for error display
            const aircraftInfoWindow = document.getElementById('aircraft-info-window');
            aircraftInfoWindow.innerHTML = `<p class="error-text" style="padding: 2rem;">Could not retrieve complete flight details. The aircraft may have landed or disconnected.</p>`;
        }
    }
    ...
    function populateAircraftInfoWindow(baseProps, plan) {
        ...
        // ================== START: FIX 3 ==================
        aircraftInfoWindow.innerHTML = `
            <div class="info-window-header">
                <h3 id="aircraft-window-title">
                    <span id="header-flight-num">${baseProps.callsign}</span>
                    <small id="header-pilot-name">${baseProps.username || 'N/A'}</small>
                </h3>
                <div class="info-window-actions">
